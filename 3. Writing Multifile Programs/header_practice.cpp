 ...
 * 
 * at the top of the header, along with an #endif at the end. 
 * 
 * This is called an "include guard". 
 * Since the header will be included into another file, and #include just pastes contents into a file, the include guard prevents the same file from being pasted multiple times into another file. 
 * This might happen if multiple files include the same header, and then are all included into the same main.cpp, for example. The ifndef checks if HEADER_EXAMPLE_H has not been defined in the file already. 
 ...
 * Another common way is to use an #pragma oncepreprocessor directive,
*/
